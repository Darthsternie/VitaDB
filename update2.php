<?php
	...
	if (mysqli_num_rows($data)>0){
		...
		if ((strcmp($roles[0],"1") == 0) or (strcmp($roles[0],"2") == 0) or (strcmp($roles[0],"3") == 0)){
			...
			$sth2 = mysqli_prepare($con,"UPDATE pspdb SET name=?,version=?,author=?,url=?,description=?,date=?,long_description=?,screenshots=?,source=?,release_page=?,trailer=?,size=?,data_size=? WHERE id=?");
